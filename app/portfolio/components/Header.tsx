    ...
    <header ref={headerRef} className={themeClasses.header}>
      ...
      {menuOpen && (
        <motion.div
          ...
          style={{ background: isLight ? "rgba(247,243,238,0.98)" : "rgba(7,17,31,0.98)", backdropFilter: "blur(16px)" }}
        >
          {/* Close strip at top — same height as header */}
          <div className="flex items-center justify-between px-4 py-4 sm:px-6" style={{ borderBottom: isLight ? "1px solid rgba(0,0,0,0.08)" : "1px solid rgba(255,255,255,0.08)" }}>
            <span className="text-sm font-semibold tracking-[0.22em] uppercase">Mahir Ahmed</span>
            <button
              type="button"
              aria-label="Close menu"
              onClick={() => setMenuOpen(false)}
              className={`inline-flex h-11 w-11 items-center justify-center rounded-full ${isLight ? "border border-slate-300 bg-white" : "border border-white/15 bg-white/5"}`}
            >
